...
var INJECTED_SCRIPT = [
    ...
    '      + "<image id=\\"ai-trace-me-" + Math.random().toString(36).substr(2,5) + "\\""',
    '      + " x=\\"0\\" y=\\"0\\" width=\\"" + w + "\\" height=\\"" + h + "\\""',
    '      + " xlink:href=\\"" + dataUrl + "\\" />"',
    '      + "</svg>";',
    '    window.parent.postMessage({type:"import-svg", data:svgStr}, "*");',
    '    _toast("Captured WYSIWYG!");',
    '  }',
    '',
    '  if (!window.html2canvas) {',
    '    _toast("Loading html2canvas...");',
    '    var s = document.createElement("script");',
    '    s.src = "https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js";',
    '    s.onload = function() {',
    '       _toast("Capturing high-res image...");',
    '       html2canvas(el, { scale: dpr, useCORS: true, allowTaint: true, backgroundColor: null }).then(sendCanvas).catch(function() { _captureFallback(el); });',
    '    };',
    '    s.onerror = function() { _toast("Failed to load render engine", true); _captureFallback(el); };',
    ...
    '}',
    '',
    'function _captureFallback(el) {',
    '  var rect = el.getBoundingClientRect();',
    '  var w = Math.ceil(rect.width), h = Math.ceil(rect.height);',
    '  var svgStr = "<svg xmlns=\\"http://www.w3.org/2000/svg\\" xmlns:xlink=\\"http://www.w3.org/1999/xlink\\" width=\\"" + w + "\\" height=\\"" + h + "\\" viewBox=\\"0 0 " + w + " " + h + "\\">";',
    ...
    '      var tx = tr.left - ox, ty = tr.top - oy;',
    '      var fontSize = parseFloat(cs.fontSize);',
    '      var anchor = cs.textAlign === "center" ? "middle" : (cs.textAlign === "right" || cs.textAlign === "end" ? "end" : "start");',
    '      var ax = anchor === "middle" ? tx + tr.width / 2 : (anchor === "end" ? tx + tr.width : tx);',
    '      svgStr += "<text x=\\"" + ax + "\\" y=\\"" + (ty + fontSize * 0.85) + "\\""',
    ...
    '        + " opacity=\\"" + cs.opacity + "\\""',
    '        + ">" + txt.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;") + "</text>";',
    ...
    '        svgStr += "<rect x=\\"" + x + "\\" y=\\"" + y + "\\" width=\\"" + w + "\\" height=\\"" + h + "\\" fill=\\"" + fill + "\\" stroke=\\"" + stroke + "\\" stroke-width=\\"" + bw + "\\" rx=\\"" + rx + "\\" />";',
    '      }',
    '      var bgImg = cs.backgroundImage;',
    '      if (bgImg && bgImg !== "none") {',
    '        var m = bgImg.match(/url\\("?([^"\\)]+)"?\\)/);',
    '        if (m) svgStr += "<image x=\\"" + x + "\\" y=\\"" + y + "\\" width=\\"" + w + "\\" height=\\"" + h + "\\" xlink:href=\\"" + m[1] + "\\" />";',
    '      }',
    ...
    '        var du = _imgToDataUrl(node);',
    '        if (du) svgStr += "<image x=\\"" + x + "\\" y=\\"" + y + "\\" width=\\"" + w + "\\" height=\\"" + h + "\\" xlink:href=\\"" + du + "\\" />";',
    '        else svgStr += "<image x=\\"" + x + "\\" y=\\"" + y + "\\" width=\\"" + w + "\\" height=\\"" + h + "\\" xlink:href=\\"" + node.src + "\\" />";',
    '        return;',
    '      }',
