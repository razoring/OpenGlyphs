...
var INJECTED_SCRIPT = [
    ...
    '  var href = a.href || "";',
    '  var dl = a.download || "";',
    '  function _import(b, ct) {',
    '    var isSvg = (b.type||"").indexOf("svg")!==-1 || ct.indexOf("svg")!==-1 || dl.toLowerCase().indexOf(".svg")!==-1 || href.indexOf(".svg")!==-1;',
    ...
    '  }',
    '  if (href.startsWith("blob:") && _blobMap[href]) {',
    '    _import(_blobMap[href], ""); return;',
    '  }',
    '  var fUrl = href;',
    ...
    '  }).then(function(obj){ _import(obj.b, obj.ct); })',
    ...
    client.get(targetUrl, options, function (targetRes) {
        ...
        if (ct.indexOf('text/html') !== -1) {
            ...
            targetRes.on('end', function () {
                ...
                if (headIdx !== -1) html = html.replace('<head>', '<head>\n' + INJECTED_SCRIPT);
                else if (html.indexOf('</body>') !== -1) html = html.replace('</body>', INJECTED_SCRIPT + '\n</body>');
                else html = INJECTED_SCRIPT + html;
                res.end(html);
            });
        } else {
            res.writeHead(targetRes.statusCode, headers);
